 ...
 *
 * 配置经 settings 命名空间 dsh-notify 读写(settingsScope),保存即生效(宿主热加载)。
 */
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ── 视觉语言(与旧版 NotifySection 的 CSS 类一一对应,全部走 DSW token)────
    const style = {
    ...
    function NotifySection({ injected }) {
      const { scope } = injected;
      // 注意:subscribe/getSnapshot 必须以箭头包住再传(裸传会丢失 this 导致渲染崩溃)
      const snapshot = react.useSyncExternalStore(
        (cb) => scope.subscribe(cb),
        () => scope.getSnapshot(),
      );
      const config = snapshot?.status === "ready" ? snapshot.value : null;
      ...
      return react_jsx_runtime.jsxs("div", { style: style.root, children: [
        ...
        error !== null ? react_jsx_runtime.jsxs("div", { style: style.error, role: "alert", children: [
          error,
          react_jsx_runtime.jsx("button", { type: "button", style: style.btnOutline, onClick: () => { setError(null); scope.load(); }, children: "重试" }),
        ] }) : null,
        ...
        writable ? null : react_jsx_runtime.jsx("p", { style: style.hint, children: "当前设置只读(由环境或组合层提供),界面不可修改。" }),
      ] });
    }
